...
int isnumberstr(const char *szString)
{
...
void print_help(const char *szCommand)
{
   ...
   printf(
     _("    -0, --fat12w7   Write a FAT12 partition Win 7 boot record to device\n"));
   printf(
     _("    -b, --fat32w7   Write a FAT32 partition Win 7 boot record to device\n"));
   printf(
   ...
   printf(
      _("    -8, --mbr7fat   Write a Windows 7 FAT MBR to device\n"));
   printf(
		   ...
		   int *piHeads)
{
   ...
   *piHeads = -1;
   ...
   while(--argc)
   {
      if( ! strcmp("--fat12w7", argv[argc]))
     *piBr = FAT12W7_BR;
      if( ! strcmp("--fat12", argv[argc]))
	 *piBr = FAT12_BR;
      else if( ! strcmp("--fat32w7", argv[argc]))
     *piBr = FAT32W7_BR;
	 ...
	 *pbWritePartitionInfo = 1;
      else if( ! strcmp("--mbr7fat", argv[argc]))
          *piBr = MBR_WIN7FAT;
